...
func RunQuery(q Query) {
	...
	case *DeleteStatement:
		s.DeleteRecord()
		fmt.Println("Ditched Her...")

	}
}

func (c *CreateStatement) StringSchema() string {
	start := "BRO, LET'S BUILD THIS PLAYBOOK "
	start += c.TableName
	start += "("
...
func (createstmt *CreateStatement) AddSchema() {
	if schema.TableName == createstmt.TableName {
		log.Printf("schema already exists for this table %s \n", createstmt.TableName)
		return
	}
	...
	if schema.PrimaryKey != "" {
		if _, ok := schema.Columns[schema.PrimaryKey]; !ok {
			...
			log.Println("no such column found to set primary_key ", schema.PrimaryKey)
			schema.PrimaryKey = ""
		}
...
func (ds *DeleteStatement) SelectAll() ([]ColumnValues, error) {
	if ds.TableName != schema.TableName {
		log.Println("Delete Statement: got different table multiple table support not yet supported", ds.TableName, schema.TableName)
	}
...
func (sel *SelectStatement) SelectAll() ([]ColumnValues, error) {
	if sel.TableName != schema.TableName {
		log.Println("Select Statement: got different table multiple table support not yet supported")
	}
	return getAllRecords()
