...
func Insert(key uint32, val []byte) (*BtreePage, error) {
	...
	if _, _, err := Search(key); err == ErrKeyNotFound {
		return nil, fmt.Errorf("key already exists cannot insert the key %w", err)
	}
...
func BtreeDFSTraverseKeys() ([][]byte, error) {
	if Init.ROOTPAGE == 0 {
		return nil, fmt.Errorf("RootPage is not set yet insert items in table")
	}
	RootNode, err := pager.GetPage(uint(Init.ROOTPAGE))
	if err != nil {
		return nil, err
